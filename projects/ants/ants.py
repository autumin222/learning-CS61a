...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        while cur_place is not beehive and i <= self.max_range:
            if self.min_range <= i and cur_place.bees:
                return rANTdom_else_none(cur_place.bees)
            cur_place, i = cur_place.entrance, i + 1
        return None
        # END Problem 3 and 4
...
# The WallAnt class
class WallAnt(Ant):
    """Wallant has more armor to protect other partners"""
    name = 'Wall'
    food_cost = 4
    implemented = True
...
class ScubaThrower(ThrowerAnt):
    """ScubaThrower can be deploied in water and 
    when it has been deploied in water, it will not get damage"""
    name = 'Scuba'
    food_cost = 6
    is_watersafe = True
    implemented = True
...
# END Problem EC
    ...
    def action(self, gamestate):
        ...
        # BEGIN Problem EC
        "*** YOUR CODE HERE ***"
        def double_damage(cur_place):
            while cur_place.exit != None:
                cur_place = cur_place.exit
                if cur_place.ant:
                    if not cur_place.ant.is_encouraged:
                        cur_place.ant.damage *= 2
                        cur_place.ant.is_encouraged = True
